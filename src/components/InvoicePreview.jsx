...
export default function InvoicePreview({ invoice, onClose, onEdit }) {
  ...
  const generateAndUploadPDF = async () => {
    ...
    try {
      ...
      // Company contact details - cleaner layout
      const contactY = height - 100;
      page.drawText(safeText(getFullAddress()), {
        x: 50,
        y: contactY,
        size: 8,
      ...
      const statusText = safeText(invoice.status.toUpperCase());
      const statusColor = invoice.status?.toLowerCase() === 'paid' ? rgb(0.13, 0.8, 0.47) : 
                         invoice.status?.toLowerCase() === 'pending' ? rgb(1, 0.65, 0) : 
                         rgb(0.8, 0.13, 0.13);
      ...
      if (invoice.dueDate) {
        ...
        page.drawText(safeText(formatInvoiceDate(invoice.dueDate)), {
          x: 50,
          y: currentY - 50,
          size: 10,
          font: helveticaFont,
      ...
      // Enhanced Invoice Items Section
      ...
      page.drawText('PREMIUM BEER PRODUCTS', {
        x: 55,
        y: currentY + 5,
        size: 14,
        font: helveticaBoldFont,
      ...
      const tableHeaders = ['Product Description', 'Qty', 'Unit Price', 'Amount'];
      ...
      tableHeaders.forEach((header, index) => {
        ...
        if (textAlign === 'center') {
          textX = tableX + (columnWidths[index] / 2) - 15;
        } else if (textAlign === 'right') {
          textX = tableX + columnWidths[index] - 12;
        }
      ...
      invoice.items.forEach((item, index) => {
        ...
        // Description
        let descText = safeText(item.product || item.name || '');
        if (item.description) {
          descText += ` - ${safeText(item.description)}`;
        }
        if (item.container && item.netContent) {
          descText += ` (${safeText(item.container)} ${safeText(item.netContent)})`;
        }
        ...
        // Quantity (center)
        page.drawText(safeText((item.quantity || 0).toString()), {
          x: tableX + (columnWidths[1] / 2) - 8,
          y: currentY - 12,
          size: 9,
          font: helveticaFont,
          color: textBlack,
        ...
        // Price (right)
        page.drawText(safeText(`Rs. ${(item.price || item.unitPrice || 0).toFixed(2)}`), {
          x: tableX + columnWidths[2] - 8,
          y: currentY - 12,
          size: 9,
          font: helveticaFont,
          color: textBlack,
        ...
        // Total (right, bold)
        page.drawText(safeText(`Rs. ${itemTotal.toFixed(2)}`), {
          x: tableX + columnWidths[3] - 8,
          y: currentY - 12,
          size: 9,
          font: helveticaBoldFont,
          color: brandGold,
      ...
      const tax = invoice.tax || (subtotal * (invoice.taxRate || 10) / 100);
      const discount = invoice.discount || 0;
      const finalTotal = invoice.total || (subtotal + tax - discount);
      
      // Totals box
      const totalsX = width - 200;
      const totalsBoxY = currentY - 20;
      ...
      const totals = [
        { label: 'Subtotal:', value: safeText(`Rs. ${subtotal.toFixed(2)}`) },
      ];
      
      if (tax > 0) {
        totals.push({ label: `Tax (${invoice.taxRate || 10}%):`, value: safeText(`Rs. ${tax.toFixed(2)}`) });
      }
      
      if (discount > 0) {
        totals.push({ label: 'Discount:', value: safeText(`-Rs. ${discount.toFixed(2)}`) });
      }
      
      totals.push({ label: 'TOTAL AMOUNT:', value: safeText(`Rs. ${finalTotal.toFixed(2)}`), isTotal: true });
      
      let totalY = currentY - 10;
      totals.forEach(total => {
        const fontSize = total.isTotal ? 14 : 10;
        const font = total.isTotal ? helveticaBoldFont : helveticaFont;
        const labelColor = total.isTotal ? brandDark : textBlack;
        const valueColor = total.isTotal ? brandGold : brandGold;
        ...
        totalY -= total.isTotal ? 25 : 18;
      });
      
      // Divider above total
      page.drawLine({
        start: { x: totalsX, y: currentY - (totals.length - 1) * 18 - 5 },
        end: { x: totalsX + 160, y: currentY - (totals.length - 1) * 18 - 5 },
        color: borderGray,
        thickness: 1,
      });
      ...
      if (invoice.notes && invoice.notes.trim()) {
        ...
        page.drawText(safeText(invoice.notes), {
          x: 50,
          y: currentY - 18,
          size: 9,
          font: helveticaFont,
      ...
      page.drawText('Thank you for choosing Peak Brew Trading!', {
        x: (width / 2) - 110,
        y: 100,
        size: 11,
        font: helveticaBoldFont,
      ...
      page.drawText('Your premier destination for quality beer distribution', {
        x: (width / 2) - 100,
        y: 85,
        size: 9,
        font: helveticaFont,
      ...
      page.drawText('Payment Terms: Net 30 days | Quality Guaranteed', {
        x: (width / 2) - 90,
        y: 55,
        size: 8,
        font: helveticaFont,
      ...
      const fileSizeKB = (pdfBlob.size / 1024).toFixed(1);
      console.log(`Premium PDF generated: ${fileSizeKB} KB`);
      ...
      notifications.update({
        ...
        message: `Professional Peak Brew invoice PDF created successfully! (${fileSizeKB} KB)`,
        loading: false,
        autoClose: 3000,
      });
